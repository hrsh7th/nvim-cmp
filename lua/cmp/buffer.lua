...
---@param bufnr number
---@param opts { keyword_length: number, keyword_pattern: string, indexing_interval: number, indexing_batch_size: number }
---@return cmp.Buffer
function buffer.new(bufnr, opts)
  ...
  self.timer = nil
  self.keyword_length = opts.keyword_length
  self.keyword_pattern = opts.keyword_pattern
  self.indexing_interval = opts.indexing_interval
  self.indexing_batch_size= opts.indexing_batch_size
...
function buffer.stop_indexing_timer(self)
  async.clear_interval(self.timer)
  self.timer = nil
  self.timer_current_line = -1
end
...
function buffer.start_indexing_timer(self)
  ...
  self.timer = async.set_interval(function()
    if self.closed then
      self:stop_indexing_timer()
      return
    end
    ...
    local batch_size = self.indexing_batch_size
  ...
  end, math.max(1, self.indexing_interval))
end
...
---@param lnum number
---@param line string
function buffer.index_line(self, lnum, line)
  local words = self.lines_words[lnum]
  if not words then
    words = {}
    self.lines_words[lnum] = words
  else
    misc.clear(words)
  end
  local word_i = 1
  ...
  while #remaining > 0 do
    ...
    local match_start, match_end = pattern.offset(self.keyword_pattern, remaining)
    if match_start and match_end then
      if (match_end - match_start) >= self.keyword_length then
        words[word_i] = remaining:sub(match_start, match_end - 1)
        word_i = word_i + 1
      end
      remaining = remaining:sub(match_end)
    else
      break
    end
  end
...
function buffer.get_words(self)
  ...
  if self.unique_words_other_lines_dirty then
    misc.clear(self.unique_words_other_lines)
  ...
  if self.unique_words_curr_line_dirty then
    misc.clear(self.unique_words_curr_line)
