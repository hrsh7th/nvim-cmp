...
---@field public cursor_pos table<number,number>|nil
---@field public hidden_wininfo table<any>|nil
local window = {}
local windows = {}
...
window.getwininfo = function(self)
    if (self.hidden) then
        return self.hidden_wininfo
    end
    return vim.fn.getwininfo(self.win)[1]
end

window.get_cursor = function(self)
    if (self.hidden) then
...
window.open = function(self, style)
    ...
    self.hidden = config.get().window.auto_hide
    if self.hidden then
        self:hide()
        return
    end
...
window.hide = function(self)
    self.hidden_wininfo = vim.fn.getwininfo(self.win)[1]
    vim.o.ei = 'all'
    self:close()
    vim.o.ei = ''
    self.hidden = true
...
window.update = function(self)
    ...
    if info.scrollable and not self.hidden then
        ...
        local thumb_offset = math.floor(info.inner_height *
            (vim.fn.getwininfo(self.win)[1].topline / self:get_content_height()))
