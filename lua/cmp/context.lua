...
context.new = function(prev_context, option)
  ...
  self.offset = pattern.offset(config.get().completion.keyword_pattern .. '$', self.cursor_before_line) or self.cursor.col
  ...
  self.replace_range = {
    ...
    ['end'] = {
      row = self.cursor.row,
      col = (function()
        local _, e = pattern.offset('^' .. config.get().completion.keyword_pattern, self.offset_after_line)
        if e then
